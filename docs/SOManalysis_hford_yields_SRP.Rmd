---
title: "Self-organizing map analysis of watershed conditions for event SRP yields at Hungerford"
...
newFolder <- paste("Data/somResults/hford/yields/SRP/", Sys.Date(), sep = "")
...
drop.info <- names(hford %>%
                     select(# Remove the non-numerical ID/INFO
                            ...
                            NO3_kg_km2, SRP_kg_km2, event_NO3_SRP))
...
# Look at plots of correlated variables
  ...
  hford %>% 
    ggplot(aes(x = rain_event_total_mm, y = turb_event_max)) +
    geom_point(shape = 1) +
    geom_abline(slope = 1) +
...
* **These decisions were tough to make and need review**
  + rain_event_total_mm and API_4d are highly correlated (88.4%), but represent different things, so I'm going to leave both for now
  + SoilTemp_pre_wet_15cm + VWC_pre_wet_15cm are negatively correlated (-72.3%), but represent different things, so I'm going to leave both for now
  + rain_int_mmPERmin_max + SoilTemp_pre_wet_15cm are correlated (70.2%), leaving both for now
...
* Trying to find a VWC variable that correlates well with GW level so that I can remove GW level vars (no GW data in 2017)
  + gw_4d_allWells and VWC_pre_wet_30cm most highly correlated (94.9%); same with VWC_pre_wet_15cm (89.6%), except slightly less linear at higher values
  ...
  + VWC_pre_wet_15cm and VWC_pre_wet_30cm highly correlated (90.5%)
...
* Soil variables
  + SoilTemp_pre_dry_15cm & equivalent at wet transect are highly correlated (99.2%); let's keep wet transect temps
  + VWC_pre_dry_15cm & VWC_pre_wet_15cm are highly correlated (85.9%); keep wet version b/c keeping most wet vs. pre versions
* MET variables
  + airT_4d and dewPoint_4d are highly correlated (97.3%), as is dewPoint_1d (91.8%); removing both dewPoints
  + airT_4d and SoilTemp_pre_wet_15cm are highly correlated (94.2%); drop airT_4d b/c we still have diff_airT_soilT
  + rain_int_mmPERmin_max + rain_int_mmPERmin_mean aren't super correlated (55.7%), but represent similar things, so removing the mean version
* Stream variables 
  + q_event_delta & q_event_max are highly correlated (95.6%); let's keep q_event_max
  + turb_kg_km2 + q_event_max are highly correlated (92%); let's keep q_event_max
  + q_event_max + q_mm are highly correlated (83.8%); let's keep q_event_max b/c NO3 & SRP yields calculated using q_mm
  + q_event_max + turb_event_max are highly correlated (78.7%); keep q_event_max
...
# Look at correlations again without dropped variables
  ...
  drop.vars <- names(hford %>% 
                       select(# Remove multipeak for now b/c we have q_4d and time_sinceLastEvent
                              ...
                              DOY,
                              # Remove these vars b/c irrelevant for SRO
                              turb_1d,
                              NO3_1d,
...
params <- read_csv(here("Data", "somParams", "params_SOM_hford_SRP.csv"))
...
for (i in 1:nrow(params)) {
      ...
      newFolder2 <- paste("Data/somResults/hford/yields/SRP/", Sys.Date(), "/SOMplots_PDFs", sep = "")
...
write_csv(datWithCluster, here(paste0(newFolder,"/","Results_", myDataSet, "_", "withClusters",".csv")))
```
<br><br>
...
datWithCluster %>% 
  ...
  mutate(var = factor(var, levels = c("rain_int_mmPERmin_mean", "rain_int_mmPERmin_max", "rain_event_hrs", "rain_event_total_mm",
                                      "q_4d", "q_event_max", "time_sinceLastEvent", "API_4d", "VWC_pre_wet_15cm", "VWC_pre_dry_30cm", "VWC_pre_dry_15cm", "SoilTemp_pre_wet_15cm",
