---
title: "Self-organizing map analysis of watershed conditions at Hungerford Brook"
...
#### Objective  
Cluster high-flow event characteristics and antecedent watershed conditions to evaluate how these factors converge as flux regimes (clusters) to produce variability in event N and P yields  
<br><br>
...
newFolder <- paste("Data/somResults/hford/yields/all/", Sys.Date(), sep = "")
...
# Look at plots of correlated variables
  ...
  hford %>%
    ...
    ggplot(aes(x = VWC, y = gw_1d_allWells, color = var)) +
    facet_wrap(~var) +
    geom_point(shape = 1) +
    geom_abline(slope = 1) +
  ...
  hford %>% 
    ggplot(aes(x = DO_pre_wet_15cm, y = DO_pre_wet_30cm)) +
    geom_point(shape = 1) +
    geom_abline(slope = 1) +
  ...
  # boxplot comparison
  hford %>% 
    pivot_longer(cols = c(q_event_max, q_mm), names_to = "var", values_to = "value") %>% 
...
* **These decisions were tough to make and need review**
  + rain_event_total_mm and API_4d are highly correlated (87.2%), but represent different things, so I'm going to leave both for now
  + airT_4d and VWC_pre_wet_30cm are highly correlated (88%); leaving both for now
  + rain_int_mmPERmin_max + airT_4d are correlated (76.2%); leaving both for now
  + VWC_pre_wet_30cm + time_sinceLastEvent are correlated (-72%); leaving both for now
  + I'm removing pre-event stream concentrations, because they keep driving the separation of a singular event that doesn't make sense
* **I feel OK about these decisions, but they should be reviewed as well**
* If the 1-d and 4-d values for a variable are highly correlated, use the 4-d value for weather vars and 1-d values for measures of catchment wetness
* Trying to find a VWC variable that correlates well with GW level so that I can remove GW level vars (no GW data in 2017)
  + gw_1d_allWells and VWC_pre_wet_30cm most highly correlated (96.3%); same with VWC_pre_wet_15cm (94.1%), except slightly less linear at higher values
  + drop gw_4d_allWells and use VWC as a proxy for GW level
  + drop VWC_pre_wet_45cm & wet transect equivalent (we'll use soil vars measured at 15cm)
  + VWC_pre_wet_15cm and VWC_pre_wet_30cm highly correlated (90.5%); VWC_pre_wet_30cm is better correlated with GW level, so going to keep VWC_pre_wet_30cm even though I'm keeping all 15cm values for other soil vars
  + gw_1d_allWells and gw_4d_allWells are highly correlated (99.5%); remove gw_4d_allWells
* Soil variables
  + SoilTemp_pre_dry_15cm & equivalent at wet transect are highly correlated (99.4%); let's keep wet transect temps
  + VWC_pre_dry_15cm & VWC_pre_wet_15cm are highly correlated (85.9%); keep wet version b/c keeping most wet vs. pre versions
* MET variables
  + airT_4d and dewPoint_4d are highly correlated (97.8%), as is dewPoint_1d (95.6%); removing both dewPoints
  + airT_4d and SoilTemp_pre_wet_15cm are highly correlated (95.4%); drop SoilTemp_pre_wet_15cm, I previously dropped airT_4d b/c still had diff_airT_soilT, but that var didn't differentiate clusters
  + rain_int_mmPERmin_max + rain_int_mmPERmin_mean aren't super correlated, but represent similar things, so removing the mean version
* Stream variables 
  + q_event_delta & q_event_max are highly correlated (94%); let's keep q_event_max
  + q_event_max + q_mm are highly correlated (86.2%); let's keep q_event_max b/c NO3 & SRP yields calculated using q_mm
* **I removed these variables after the initial SOM run b/c they did not differentiate clusters**
  + diff_airT_soil_T; PET_mmHR; Redox_pre_dry_15cm
...
# Look at correlations again without dropped variables
  ...
  drop.vars <- names(hford %>% 
                       select(# Remove multipeak for now b/c we have q_4d and time_sinceLastEvent
                              ...
                              # Remove all pre-event soil var means and keep the aggregate "dry" and "wet" ones
                              contains("HWp"),
                              # Let's use 15cm values for SoilTemp for now; remove all 30cm & 45cm aggreate soil vars except VWC
                              ...
                              # For now, let's drop the aggreate soil vars for the dry site and keep the wet ones
                                # contains("pre_dry"),
                              # Drop the _1d version of variables climate variables
                                airT_1d,
                                dewPoint_1d,
                                solarRad_1d,
                              # Drop the _4d version of measures of wetness
                                gw_4d_allWells,
                                q_4d,
                              ...
                              # Removing DOY b/c other variables have seasonal cycles
                                DOY,
                              # Remove antecedent stream concentrations?
                                turb_1d,
                                SRP_1d,
                                NO3_1d,
                              # These should be removed b/c they're not actually complete variables we care about
                                pvol,
                                pmax
...
```{r, include = TRUE, warning = FALSE, fig.width = 10, fig.height = 8, out.width = "90%"}
myData <- hford_drop %>% 
  # Keep only complete observations/rows (no NAs in any of the columns)
  na.omit()
...
params <- read_csv(here("Data", "somParams", "params_SOM_hford_all.csv"))
...
for (i in 1:nrow(params)) {
      ...
      newFolder2 <- paste("Data/somResults/hford/yields/all/", Sys.Date(), "/SOMplots_PDFs", sep = "")
...
best_summ_top <-
  ResultsDF %>%
  select(Run, rows, cols, Nodes, Clusters, npF, QE) %>%
  filter(Run %in% c(25, 50, 37))

# noquote("These were the top runs for each high-performing (top 50%) cluster #:")
best_summ_top %>% 
...
# Select the best 5 cluster
best_summ_top5 <-
  best_summ_top %>% 
  slice(3:3)

# Which run is best?
  best_run_3cl = best_summ_top3$Run
...
datWithCluster <- clustAssignmDF %>% 
  ...
  type_convert() %>% 
  filter(Run == best_run_3cl | Run == best_run_4cl | Run == best_run_5cl) %>% 
  ...
  rename(clust_wat_3cl = `3`, clust_wat_4cl = `4`, clust_wat_5cl = `5`) %>% 
  ...
  select(site:season, clust_wat_3cl, clust_wat_4cl, clust_wat_5cl, everything()) %>% 
  select(-obs) %>% 
  # Add groundwater data
  left_join(hford %>% select(site, event_start, starts_with(c("gw_1d", "gw_4d")))) %>% 
  select(site:turb_event_max, starts_with("gw"), everything())
...
##### 3 cluster model - examine boxplots of independent variables by cluster
...
datWithCluster %>% 
  ...
  ggplot(aes(x = clust_wat_3cl, y = value, group = clust_wat_3cl)) +
...
datWithCluster %>% 
  ...
  mutate(var = factor(var, levels = c("rain_int_mmPERmin_max","rain_int_mmPERmin_mean", "rain_event_hrs", "rain_event_total_mm", "pmax_pvol", "cv_rain",
                                      "API_4d", "q_1d", "q_4d", "q_event_max", "time_sinceLastEvent", "VWC_pre_wet_30cm",
                                      "DO_pre_dry_15cm", "DO_pre_wet_15cm", "Redox_pre_dry_15cm", "Redox_pre_wet_15cm", 
                                      "solarRad_4d", "PET_mmHR", "diff_airT_soilT", "airT_4d"))) %>%
