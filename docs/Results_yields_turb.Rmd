---
title: <font size = "5"> Visualizing all SOM event turbidity yield results </font>
...
### Objective  
##### Visualize results of SOM clustered flux regimes on event turbidity yields from Hungerford and Wade Brooks
<br><br>
...
# Read in data
  ...
  # Turbidity
  som_hf <- read_csv(here("Data", "somResults", "hford", "yields", "turb", "2021-04-09", "Results_hford_withClusters.csv"))
  som_wd <- read_csv(here("Data", "somResults", "wade", "yields", "turb", "2021-04-13", "Results_wade_withClusters.csv"))
```
...
# HFORD
  ...
  p_hf_5 <- som_hf %>%
    ...
    ggplot(aes(x = month_day_rep, y = turb_kg_km2, fill = as.factor(clust_wat_4cl))) +
      facet_wrap(~year, scales = "free_x", ncol = 2) +
  ...
  p_hf_6 <- som_hf %>%
    ...
    ggplot(aes(x = month_day_rep, y = turb_kg_km2, fill = as.factor(clust_wat_5cl))) +
      facet_wrap(~year, scales = "free_x", ncol = 2) +
      geom_bar(stat = "identity") +
      # scale_fill_manual(name = "Cluster",
      #                   values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7"),
  ...
  p_hf_7 <- som_hf %>%
    ...
    ggplot(aes(x = month_day_rep, y = turb_kg_km2, fill = as.factor(clust_wat_6cl))) +
      facet_wrap(~year, scales = "free_x", ncol = 2) +
...
# HFORD
  ...
  pl_hf_x1 <- som_hf %>%
    mutate(year = year(event_start)) %>% 
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(clust_wat_4cl), y = turb_kg_km2, color = factor(clust_wat_4cl))) +
    facet_wrap(~year) +
    geom_boxplot() +
  ...
  pl_hf_x2 <- som_hf %>%
    mutate(year = year(event_start)) %>% 
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(clust_wat_5cl), y = turb_kg_km2, color = factor(clust_wat_5cl))) +
    facet_wrap(~year) +
    geom_boxplot() +
    scale_color_manual(name = "Cluster",
                      values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7")) +
  ...
  pl_hf_x3 <- som_hf %>%
    mutate(year = year(event_start)) %>% 
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(clust_wat_6cl), y = turb_kg_km2, color = factor(clust_wat_6cl))) +
    facet_wrap(~year) +
    geom_boxplot() +
    scale_color_manual(name = "Cluster",
                      values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7")) +
...
# HFORD
  ...
  annYields <-
    ...
    mutate(year = year(event_start)) %>% 
    group_by(year) %>% 
    summarize(ann_turb_yield = sum(turb_kg_km2, na.rm = TRUE)) %>% 
    ungroup()
  ...
  p_hf_8 <- som_hf %>%
    ...
    summarize(tot_yield = sum(turb_kg_km2, na.rm = TRUE)) %>% 
    ungroup() %>% 
    # Exclude 2017 events  
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(year), y = tot_yield, fill = factor(clust_wat_4cl))) +
      ...
      scale_fill_manual(name = "Cluster",
                        values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7")) +
      ylab(expression(atop(Event~turb.~yield, "during ice-free season"))) +
      xlab("Year") +
      theme1 +
      theme(legend.position = "none",
            axis.text.x = element_blank(),
  ...
  p_hf_11 <- som_hf %>%
    # Calculate the cummulative sum of yields for each solute, year, and cluster #
    mutate(year = year(event_start)) %>% 
    # pivot_longer(cols = c(turb_kg_km2, turb_kg_km2), names_to = "var", values_to = "value") %>% 
    # group_by(year, cluster, var) %>% 
    group_by(year, clust_wat_4cl) %>% 
    summarize(tot_turb_yield = sum(turb_kg_km2, na.rm = TRUE)) %>% 
    ungroup() %>% 
    # Calculate proportion of total annual yield
    full_join(annYields) %>% 
    mutate(prop_turb_yield = tot_turb_yield/ann_turb_yield) %>% 
    # Exclude 2017 events
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(year), y = prop_turb_yield*100, fill = factor(clust_wat_4cl))) +
  ...
  p_hf_12 <- som_hf %>%
    ...
    # Exclude 2017 events
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(year), y = prop_turb_yield*100, fill = factor(clust_wat_5cl))) +
      # facet_wrap(~var, ncol = 2, scales = "free_y", labeller = labeller(var = labels_var)) +
      geom_bar(position = "stack", stat = "identity") +
      # scale_fill_manual(name = "Cluster",
      #                   values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7"),
  ...
  p_hf_13 <- som_hf %>%
    ...
    # Exclude 2017 events
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(year), y = prop_turb_yield*100, fill = factor(clust_wat_6cl))) +
      # facet_wrap(~var, ncol = 2, scales = "free_y", labeller = labeller(var = labels_var)) +
      geom_bar(position = "stack", stat = "identity") +
      # scale_fill_manual(name = "Cluster",
      #                   values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7"),
