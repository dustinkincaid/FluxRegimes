---
title: <font size = "5"> Self-organizing map analysis for Wade </font>
...
# Look at plots of correlated variables
  # VWC at 15cm vs 30cm
  wade %>% 
    ggplot(aes(x = VWC_pre_wet_30cm, y = VWC_pre_wet_15cm)) +
    geom_point(shape = 1) +
    geom_abline(slope = 1) +
    geom_smooth(method = "lm") +
    theme_classic()
...
params <- read_csv(here("Data", "somParams", "params_SOM_wade_2020-12-08.csv"))
...
for (i in 1:nrow(params)) {
      ...
      newFolder2 <- paste("Data/somResults/wade/", Sys.Date(), "/SOMplots_PDFs", sep = "")
...
# CHOOSE BEST LATTICE CONFIGURATION ----
  ...
  ResultsDF %>% 
    ...
    xlab("Lattice config. & clusters (rows x cols _ clusters _ run)") +
    theme_bw() +
...
datWithCluster %>% 
  ...
  summarize(median = median(value, na.rm = T)) %>% 
  # Order the vars n var
  mutate(var = factor(var, levels = c("turb_event_max", "multipeak", "q_mm", "rain_event_total_mm", "rain_event_hrs", "rain_int_mmPERmin_mean",
                                      "rain_int_mmPERmin_max", "SRP_1d", "NO3_1d", "q_4d", "time_sinceLastEvent", "API_4d", "Redox_pre_wet_15cm",
                                      "DO_pre_wet_15cm", "VWC_pre_wet_15cm", "SoilTemp_pre_wet_15cm", "solarRad_4d", "PET_mmHR", "diff_airT_soilT", "DOY"))) %>%
...
ggsave(here("Plots", "SOM_wd_zScoresByCluster.pdf"), device = "pdf", width = 6, height = 6, units = "in", dpi = 150)
...
ggsave(here("Plots", "SOM_wd_clustersBySeason.pdf"), device = "pdf", width = 4, height = 4, units = "in", dpi = 150)
```
<br><br>

```{r, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 10, out.width = "100%"}
...
ggsave(here("Plots", "SOM_wd_eventsByCluster.pdf"), device = "pdf", width = 10, height = 6, units = "in", dpi = 150)
```
<br><br>
...
labels_var <- c(NO3_kg_km2 = "Nitrate", SRP_kg_km2 = "SRP")

# Grouped bar graph (cluster on x-axis)
# datWithCluster %>% 
...
#   ungroup() %>% 
#   filter(year != 2017) %>% 
#   ggplot(aes(x = factor(cluster), y = tot_yield, fill = factor(year))) +
#     facet_wrap(~var, ncol = 1, scales = "free_y", labeller = labeller(var = labels_var)) +
#     geom_bar(position = position_dodge(preserve = "single"), stat = "identity") +
#     # ylab(expression(Annual~event~yield~(kg~NO[3]^-{}-N~or~SRP-P~km^{-2}))) +
...
ggsave(here("Plots", "SOM_wd_ratiosBYWaterYield.pdf"), device = "pdf",  width = 4, height = 4, units = "in", dpi = 150)
```
<br><br>
...
pl_hifi_no3 <- 
  datWithCluster %>% 
  left_join(hyst %>% filter(site == "Wade"), by = c("site", "event_start")) %>%
  ggplot() +
...
test_alt <- grid.arrange(arrangeGrob(pl_HI_v_FI_alt, left = y.grob, bottom = x.grob))
ggsave(here("Plots", "HIFI_wd.pdf"), plot = test_alt, device = "pdf", width = 6, height = 3.5, units = "in", dpi = 300)    
```
<br><br>
