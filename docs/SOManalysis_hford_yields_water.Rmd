---
title: <font size = "5"> Self-organizing map analysis for event water yields at Hungerford
  </font>
author: "Dustin Kincaid"
date: "04/07/2021<br><br>"
output:
  html_document:
    df_print: paged
...
#### Objective  
Cluster high-flow event characteristics and antecedent watershed conditions to evaluate how these factors converge as flux regimes (clusters) to produce variability in event water yields  
<br><br>
...
newFolder <- paste("Data/somResults/hford/yields/water/", Sys.Date(), sep = "")
...
drop.info <- names(hford %>% 
                     select(# Remove the non-numerical ID/INFO
                            ...
                            q_mm, q_event_max, q_event_delta, q_event_dQRate_cmsPerHr))
...
hford %>% 
  ...
  select(-c(NO3_kg_km2, SRP_kg_km2, event_NO3_SRP, turb_kg_km2,
         NO3_1d, SRP_1d, turb_1d, turb_event_max, 
         starts_with("DO_"), starts_with("Redox"), multipeak)) %>% 
...
* **These decisions were tough to make and need review**
  + rain_event_total_mm and API_4d are highly correlated (87.2%), but represent different things; I'm going to leave both for now, and look at PCA later
  + VWC_pre_dry_15cm and VWC_pre_wet_15cm are positively correlated (73.4%); I'm going to leave both for now, and look at PCA later
  + rain_int_mmPERmin_max & SoilTemp_pre_wet_15cm positively correlated (70.3%); I'm going to leave both for now, and look at PCA later  
...
* If the 1-d and 4-d values for a variable are highly correlated, use the 4-d value
  ...
  + airT_1d and airT_4d are highly correlated (92.9%); removing airT_1d
  + q_1d and q_4d highly correlated (86.4%); removing q_1d
  + solarRad_1d and solarRad_4d are positively correlated (73.2%); drop solarRad_1d per rule above  
...
# Look at correlations again without dropped variables
  ...
  drop.vars <- names(hford %>% 
                       select(NO3_kg_km2, SRP_kg_km2, event_NO3_SRP, turb_kg_km2,
                              NO3_1d, SRP_1d, turb_1d, turb_event_max, 
                              starts_with("DO_"), starts_with("Redox"), multipeak,
...
# scale = TRUE = correlation matrix; scale = FALSE = covariance matrix
  ...
  pcaCharts(PCA)
```
Leaving all vars for now, but might want to consider dropping either API_4d or rain_event_total_mm b/c nearly on same vector
<br><br>
...
params <- read_csv(here("Data", "somParams", "params_SOM_hford_water.csv"))
...
for (i in 1:nrow(params)) {
      ...
      newFolder2 <- paste("Data/somResults/hford/yields/water/", Sys.Date(), "/SOMplots_PDFs", sep = "")
...
Here are the top 30% of runs based on npF
