---
title: <font size = "5"> Visualizing all SOM yield results </font>
author: "Dustin Kincaid"
date: "04/14/2021<br><br>"
...
### Objective  
##### Visualize results of SOM clustered flux regimes on water, NO3, SRP, and turbidity event yields from Hungerford and Wade Brooks
<br><br>
...
# Read in data
  ...
  # Water
  som_wat_hf <- read_csv(here("Data", "somResults", "hford", "yields", "water", "2021-04-07", "Results_hford_withClusters.csv"))
  som_wat_wd <- read_csv(here("Data", "somResults", "wade", "yields", "water", "2021-04-07", "Results_wade_withClusters.csv"))
  # NO3
  som_no3_hf <- read_csv(here("Data", "somResults", "hford", "yields", "NO3", "2021-04-08", "Results_hford_withClusters.csv"))
  som_no3_wd <- read_csv(here("Data", "somResults", "wade", "yields", "NO3", "2021-04-12", "Results_wade_withClusters.csv"))
  # SRP
  som_srp_hf <- read_csv(here("Data", "somResults", "hford", "yields", "SRP", "2021-04-08", "Results_hford_withClusters.csv"))
  som_srp_wd <- read_csv(here("Data", "somResults", "wade", "yields", "SRP", "2021-04-12", "Results_wade_withClusters.csv"))
  # Turbidity
  som_turb_hf <- read_csv(here("Data", "somResults", "hford", "yields", "turb", "2021-04-09", "Results_hford_withClusters.csv"))
  som_turb_wd <- read_csv(here("Data", "somResults", "wade", "yields", "turb", "2021-04-13", "Results_wade_withClusters.csv"))  
...
# ggsave(here("Plots", "som_wat_hf_clustersBySeason.pdf"), device = "pdf", width = 4, height = 4, units = "in", dpi = 150)
```
...
# HFORD
  ...
  p_hf_5 <- som_wat_hf %>% 
    ...
    ggplot(aes(x = month_day_rep, y = q_mm, fill = as.factor(clust_wat_4cl))) +
      facet_wrap(~year, scales = "free_x", ncol = 2) +
  ...
  p_hf_6 <- som_wat_hf %>% 
    ...
    ggplot(aes(x = month_day_rep, y = q_mm, fill = as.factor(clust_wat_5cl))) +
      facet_wrap(~year, scales = "free_x", ncol = 2) +
      geom_bar(stat = "identity") +
      # scale_fill_manual(name = "Cluster",
      #                   values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7"),
  ...
  p_hf_7 <- som_wat_hf %>% 
    ...
    ggplot(aes(x = month_day_rep, y = q_mm, fill = as.factor(clust_wat_6cl))) +
      facet_wrap(~year, scales = "free_x", ncol = 2) +
...
# ggsave(here("Plots", "som_wat_hf_eventsByCluster.pdf"), device = "pdf", width = 10, height = 6, units = "in", dpi = 150)
```
...
# HFORD
  ...
  pl_hf_x1 <- som_wat_hf %>% 
    mutate(year = year(event_start)) %>% 
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(clust_wat_4cl), y = q_mm, color = factor(clust_wat_4cl))) +
    facet_wrap(~year) +
    geom_boxplot() +
  ...
  pl_hf_x2 <- som_wat_hf %>% 
    mutate(year = year(event_start)) %>% 
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(clust_wat_5cl), y = q_mm, color = factor(clust_wat_5cl))) +
    facet_wrap(~year) +
    geom_boxplot() +
    scale_color_manual(name = "Cluster",
                      values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7")) +
  ...
  pl_hf_x3 <- som_wat_hf %>% 
    mutate(year = year(event_start)) %>% 
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(clust_wat_6cl), y = q_mm, color = factor(clust_wat_6cl))) +
    facet_wrap(~year) +
    geom_boxplot() +
    scale_color_manual(name = "Cluster",
                      values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7")) +
...
# HFORD
  ...
  annYields <-
    ...
    mutate(year = year(event_start)) %>% 
    group_by(year) %>% 
    summarize(ann_q_yield = sum(q_mm, na.rm = TRUE)) %>% 
    ungroup()
  ...
  p_hf_8 <- som_wat_hf %>% 
    ...
    summarize(tot_yield = sum(q_mm, na.rm = TRUE)) %>% 
    ungroup() %>% 
    # Exclude 2017 events  
    filter(year != 2017) %>% 
  ...
  p_hf_11 <- som_wat_hf %>% 
    ...
    group_by(year, clust_wat_4cl) %>% 
    summarize(tot_q_yield = sum(q_mm, na.rm = TRUE)) %>% 
    ungroup() %>% 
    # Calculate proportion of total annual yield
    full_join(annYields) %>% 
    # mutate(prop_yield = tot_yield/ann_yield) %>% 
    mutate(prop_q_yield = tot_q_yield/ann_q_yield) %>% 
    # Exclude 2017 events
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(year), y = prop_q_yield*100, fill = factor(clust_wat_4cl))) +
  ...
  p_hf_12 <- som_wat_hf %>% 
    ...
    # Exclude 2017 events
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(year), y = prop_q_yield*100, fill = factor(clust_wat_5cl))) +
      # facet_wrap(~var, ncol = 2, scales = "free_y", labeller = labeller(var = labels_var)) +
      geom_bar(position = "stack", stat = "identity") +
      # scale_fill_manual(name = "Cluster",
      #                   values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7"),
  ...
  p_hf_13 <- som_wat_hf %>% 
    ...
    # Exclude 2017 events
    filter(year != 2017) %>% 
    ggplot(aes(x = factor(year), y = prop_q_yield*100, fill = factor(clust_wat_6cl))) +
      # facet_wrap(~var, ncol = 2, scales = "free_y", labeller = labeller(var = labels_var)) +
      geom_bar(position = "stack", stat = "identity") +
      # scale_fill_manual(name = "Cluster",
      #                   values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7"),
