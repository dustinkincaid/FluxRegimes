...
# HUNGERFORD ----  
  ...
  # SET UP THE GRID/LATTICE FOR THE SOM ----
    ...
    VesantoNodes_hf = round(5 * sqrt(nrow(dat_hf)), 0)
    ...
    PCA_hf <- prcomp(dat_hf, scale = TRUE, center = TRUE)
    eigenValues_hf <- PCA_hf$sdev^2
    first_hf <- nth(eigenValues_hf, 1)
    second_hf <- nth(eigenValues_hf, 2)
    gridRatio_hf <- round((first_hf/second_hf), 1)
...
params <- read.csv("params_SOM.csv",na.strings=".", header=T)
...
for (i in 1:nrow(params)) {
      ...
      myFileName2 <- paste(myRun,"SOMplots_LkChamp",myDataSet,wghtMeth,normMeth, paste0(rows,"x", cols, myTopo), paste0(nclusters,"cl"), sep = "_")
      pdf(file=myFileName2, height=8, width=10)  #dev.off() is at 400
...
write.csv(ResultsDF, paste0("Results_", myDataSet,"_",nclusters,"_cl",".csv"))
...
write.csv(clustAssignmDF, paste0("ClustAssign_",myDataSet,"_",nclusters,"_cl",".csv"))    
